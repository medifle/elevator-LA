...
def plot_group_bar(scheme: str, y1: List[float], y2: List[float], n1: int, n2: int) -> None:
    """
    ...
    plt.bar(x1, y1, color="#646FFB", label="N = %d" % n1)
    plt.bar(x2, y2, color="#EF553B", label="N = %d" % n2)
...
def plot_bar(scheme: str, y: List[float], label_name: str, label_value: float) -> None:
    plt.clf()
    precision = 4
    x = [1, 2, 3, 4, 5, 6]
